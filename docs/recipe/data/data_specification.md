...
流水线在校验阶段会使用 `recipe.data.DataChecker` 注册的规则；通用规则见 [`recipe/data/rules.py`](https://github.com/verl-project/rl-insight/blob/v0.2.x/recipe/data/rules.py)，VeRL 日志规则见 [`recipe/data/verl_log_rules.py`](https://github.com/verl-project/rl-insight/blob/v0.2.x/recipe/data/verl_log_rules.py)。**具体校验项以代码为准**，部分规则可能尚未接入 `DataChecker.rules`，文档仅描述数据侧约定。
...
└── <role>/
    └── prof_*.json.gz
```
参考：[`./data/recipe/torch_data`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/torch_data)
...
└── <role>/
    └── *_ascend_pt/
        ...
        └── ASCEND_PROFILER_OUTPUT/
            └── trace_view.json
```
参考：[`./data/recipe/mstx_data`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/mstx_data)
...
```
参考：[`./data/recipe/summary_event_data`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/summary_event_data)
...
`DataEnum.VERL_LOG` 对 **单个** VeRL 训练 `.log` 文件做存在性与关键指标子串校验（例如 `DataChecker` 或 [`tests/recipe/data/check_verl_log.py`](https://github.com/verl-project/rl-insight/blob/v0.2.x/tests/recipe/data/check_verl_log.py)）。路径必须是文件，不能是目录。

### 5.1 校验规则（以代码为准）

1. **存在与路径**（`VerlLogExistRule`）：扩展名为 `.log`，文件非空，且能被识别为 VeRL 日志：文件名中含 `verl`（不区分大小写），或文件开头约 64KiB 内容中含 `verl`。
2. **关键子串**（`VerlLogKeyParamsRule`）：日志正文（读取至多约 2MiB，**不区分大小写**）须**同时包含**以下子串，定义见 [`recipe/data/verl_log_rules.py`](https://github.com/verl-project/rl-insight/blob/v0.2.x/recipe/data/verl_log_rules.py) 中 `DEFAULT_REQUIRED_KEYWORDS`：
...
仓库 [`data/recipe/verl_data/`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/verl_data/) 下提供：
...
GMM 热力图输入类型为 `DataEnum.GMM_DATA`。**路径约定、参数与示意图**见 [`docs/recipe/overview/gmm_heatmap_quickstart.md`](https://github.com/verl-project/rl-insight/blob/v0.2.x/docs/recipe/overview/gmm_heatmap_quickstart.md)。本节补充数据侧目录与文件格式说明。
...
参考（仓库内**最小可解析**示例，体量极小，便于测试与文档对照）：[`../../../data/recipe/gmm_data`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/gmm_data)
...
`GmmDataRule`（见 [`recipe/data/rules.py`](https://github.com/verl-project/rl-insight/blob/v0.2.x/recipe/data/rules.py)）要求：
...
Memory 模块输入类型为 `DataEnum.ASCEND_MEMORY`（CLI：`timeline.parser.type=memory`）。用户侧快速入门见 [Memory Quickstart](https://github.com/verl-project/rl-insight/blob/v0.2.x/docs/recipe/overview/memory_quickstart.md)，开发者指南见 [Memory Guide](https://github.com/verl-project/rl-insight/blob/v0.2.x/docs/recipe/developer_guides/memory_guide.md)。
...
参考：[`./data/recipe/memory_data`](https://github.com/verl-project/rl-insight/tree/v0.2.x/data/recipe/memory_data)
...
Memory 模块输出类型为 `DataEnum.MEMORY_SUMMARY`，为 `pd.DataFrame`，每行对应一条内存分配/释放记录。完整字段说明见 [Memory Guide](https://github.com/verl-project/rl-insight/blob/v0.2.x/docs/recipe/developer_guides/memory_guide.md) 中 `MemoryEventRow` 定义。
...
`DataEnum.ASCEND_MEMORY` 当前未注册校验规则（`DataChecker.rules` 中为空列表），输入校验由 `MemoryClusterParser.parse_analysis_data()` 内部的文件存在性检查承担。如需增加校验规则，参考 [DataRule 扩展说明](https://github.com/verl-project/rl-insight/blob/v0.2.x/docs/recipe/developer_guides/rule_extending_guide.md)。
