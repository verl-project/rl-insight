...
def hub() -> Any:
    ...
    instance._diagnostic_registry = MagicMock()
...
def test_init_should_configure_collectors_and_register_scrape_target_when_created(
    ...
    registry_factory = MagicMock(side_effect=[registry, diagnostic_registry])
    ...
    assert registry_factory.call_args_list == [
        call(namespace="trainer"),
        call(namespace="trainer", subsystem="diagnostics"),
    ]
    ...
    assert instance._diagnostic_registry is diagnostic_registry
...
def test_apply_event_should_dispatch_metric_when_kind_is_supported(
    ...
    hub._diagnostic_registry.count.assert_called_once_with(
        "events_applied",
    ...
    hub._diagnostic_registry.value.assert_called_once()
...
def test_apply_event_should_export_trace_when_trace_collection_is_enabled(
    ...
    hub._diagnostic_registry.count.assert_called_once()
    hub._diagnostic_registry.value.assert_called_once()
...
def test_apply_event_should_raise_value_error_when_event_is_invalid(
    ...
    hub._diagnostic_registry.count.assert_called_once()
    hub._diagnostic_registry.value.assert_not_called()
