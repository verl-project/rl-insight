...
from recipe.parser.parser import CLUSTER_PARSER_REGISTRY, register_cluster_parser
...
from recipe.visualizer.visualizer import (
...
class TestMstxClusterParser:
    ...
    def test_get_data_map_sorts_by_legacy_datetime_segment(self, tmp_path):
        """Directory ordering should follow the legacy date+time underscore layout."""
        parser = MstxClusterParser(
            {
                Constant.INPUT_PATH: str(tmp_path),
                Constant.RANK_LIST: "all",
        ...
        first = tmp_path / "role_a" / "20250101_230000_ascend_pt"
        second = tmp_path / "role_a" / "20250102_010000_ascend_pt"
...
class TestVisualizerFunctions:
    ...
    @patch("recipe.visualizer.timeline_visualizer.go.Figure")
...
class TestIntegration:
    ...
    def test_full_pipeline_with_mock_data(self, mock_mstx_profiler_structure, tmp_path):
        ...
        with patch("recipe.visualizer.timeline_visualizer.go.Figure") as mock_figure:
            mock_fig = MagicMock()
            mock_figure.return_value = mock_fig
    ...
    @patch("recipe.pipeline.offline_insight_pipeline.DataChecker.run")
    @patch("recipe.pipeline.offline_insight_pipeline.get_cluster_parser_cls")
    @patch("recipe.pipeline.offline_insight_pipeline.get_cluster_visualizer_cls")
